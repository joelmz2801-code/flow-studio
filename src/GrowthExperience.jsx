...
const TASKS = [
  ...
  { id: 'social', label: '社媒封面', kicker: '一眼抓住注意力', art: 'social', ratio: '3:4', style: '编辑设计', prompt: '制作一张城市周末指南封面：明快编辑排版感，咖啡店与街景拼贴，奶油黄和钴蓝配色，年轻轻松，3:4 竖版，画面不要出现乱码文字。' },
...
export default function GrowthExperience({ activeView }) {
  ...
  const applyTask = (task) => {
    ...
    setNotice(`已准备好「${task.label}」：检查内容后点击发送`)
  }
      ...
      {onboardingVisible && (
        <section className="growth-onboarding" aria-labelledby="growth-title">
          <div className="growth-copy">
            <span className="growth-eyebrow">第一次使用？从这里开始</span>
            <h1 id="growth-title">两分钟完成第一张作品</h1>
            <p>你不需要懂提示词。选一个方向，我会准备好描述、画幅和风格，你只需要检查一下，然后点击发送。</p>
          </div>
          ...
          <p className="growth-reassurance">可随时修改提示词、画幅和风格，生成前不会自动提交。</p>
        </section>
      )}
